		...
		for (int r = 0; r < board.size(); r++) {
			for (int c = 0; c < board.size(); c++) {
				...
				if (visited[r][c] == false) {
					...
					// If current letter matches first letter of remaining String 
					// and also is a neighbor of previous BoardCell
					if (board.getFace(r, c).substring(0, 1).equals(String.valueOf(remaining.charAt(0))) 
						...
						// If current letter can form the given word, then return the word
