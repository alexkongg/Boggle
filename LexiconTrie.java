...
public class LexiconTrie implements LexiconInterface {
	...
	/**
	 * Helper function for load that takes in a string strItem and
	 * a LetterNode w and constructs a trie path for strItem starting from w.
	 * 
	 * @param strItem
	 *                is the word to be constructed in the Trie
	 *                
	 * @param letter
	 *                is the letter from which strItem is to be constructed
	 */
	private void loadHelper(String strItem, LetterNode letter) {
		...
		if (strItem.equals("")) {
			...
			for (int x = 0; x < letter.myChildren.size(); x++) {

				// If w already has a child LetterNode equal to the first letter of strItem
				if (letter.myChildren.get(x).toString().equals(strItem.substring(0,1))) {
					LetterNode nextLetter = letter.myChildren.get(x);
			...
			LetterNode newLetter = new LetterNode(strItem.substring(0,1), letter);
			letter.myChildren.add(newLetter);
	...
	/**
	 * Helper function for containsPrefix that takes in a String s
	 * and a LetterNode letter and verifies that the first letter of s
	 * matches the LetterNode letter. If this does not hold; returns false.
	 * 
	 * @param s
	 *                is the prefix being checked if it is in the dictionary
	 *                
	 * @param letter
	 *                is compared to the first letter of s to see if s is in the dictionary
	 */
	...
	/**
	 * Helper function for contains that takes in a String s
	 * and a LetterNode letter and verifies that the first letter of s
	 * matches the LetterNode letter. If this does not hold; returns false.
	 * 
	 * @param s
